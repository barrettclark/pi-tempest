...
def mb_to_inhg(mb: Optional[float]) -> Optional[float]:
    if mb is None:
        return None
    return round(mb * 0.02953, 3)


def mm_to_in(mm: Optional[float]) -> float:
    if mm is None:
        return 0.0
    return round(mm * 0.0393701, 3)


def dew_point_f(temp_c: Optional[float], rh: Optional[float]) -> Optional[float]:
...
def feels_like_f(temp_c: Optional[float], rh: Optional[float], wind_ms: Optional[float]) -> Optional[float]:
    """
...
def degrees_to_cardinal(deg: Optional[int]) -> Optional[str]:
    if deg is None:
        return None
